	...
	if eventTriggers:
		...
		match event.type:
			EventTypes.EventType.DIALOGUE:
				...
				eventTriggers = false
			EventTypes.EventType.SCENE:
				var sceneEvent:SceneEventResource = event
				animationPlayer.play(event.animName)
				print(event.animName)
			_: 
				print("Undefined Event Type")
		pass
	pass
